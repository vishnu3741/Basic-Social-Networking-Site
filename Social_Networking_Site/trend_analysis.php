...
.post .media iframe {
 ...
 // echo $_SESSION["user_name"]." user name<br>";
  ...
  $_SESSION["tag"] = $tag;
  //echo $tag." tag";
  echo "<h1>Posts on the tag #".$tag."</h1>";
  $result = pg_query($dbconn, "SELECT post,time_stamp,sender_id,accounts.name,feed_messages.location as location1,accounts.location as location2 FROM feed_messages,accounts WHERE tags @> '{{$tag}}' and accounts.id = feed_messages.sender_id ORDER BY time_stamp DESC LIMIT 20");
